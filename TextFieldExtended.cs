...
{
    public class TextFieldExtended : MonoBehaviour
    {
        /// <summary>
        ...
        public string Text
        {
            get
            {
                if (_text == null)
                {
                    ...
                    _text = FromDisplayText(TextBlock.Text, displayTags);
                }
                return _text;
            }
            set
            {
                ...
                string displayText = ToDisplayText(newText, displayTags);
        ...
        public string GetSelectedString()
        {
            ...
            return FromDisplayText(subStr, displayTags);
        }
        ...
        public void Insert(string toInsert)
        {
            ...
            else
            {
                ...
                // Update the text
                Text = FromDisplayText(newText, displayTags);
        ...
        // Coffee: Update the display tags, primarily used to make the text monospace, but could be used for any TMP tags
        public void UpdateTags(string tag, bool add)
        {

            if(add)
            {

                if (!displayTags.Contains(tag))
                {
            ...
            else
            {
                ...
                if(displayTags.Contains(tag))
                {
                    ...
                    int index = displayTags.IndexOf(tag);
                    displayTags = (index < 0) ? displayTags : displayTags.Remove(index, tag.Length);

                }
            }

            string displayText = ToDisplayText(Text, displayTags);
            TextBlock.Text = displayText;

        }

        // Coffee: Retrive tags for use in other text bodies, used in the protected text display
        ...
        private void Delete(TextPosition left, TextPosition right)
        {
            ...
            newText = FromDisplayText(newText, displayTags);
        ...
        /// Coffee: Always add the tag(s) before the new display string
        /// </summary>
        public static string ToDisplayText(string s, string tag)
        {
            ...
            {
                return $"{tag}{s}{CharExtensions.EmptyWidthSpace}";
            }
            else
            {
                return tag + s;
        ...
        /// Coffee: Remove tag(s) if they are preset for the "output" string Text
        /// </summary>
        public static string FromDisplayText(string s, string tag)
        {
            if (string.IsNullOrEmpty(s))
            {
                return string.Empty;
            ...
            if (s[s.Length - 1].IsEmptySpace())
            {
                if (s.Contains(tag))
                {
                    ...
                    return s.Substring(tag.Length, s.Length - tag.Length - 1);
            ...
            else
            {
                if(s.Contains(tag))
                {
                    ...
                    return s.Substring(tag.Length, s.Length - tag.Length);
